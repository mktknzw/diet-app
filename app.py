...
# 🔑 セキュリティ設定 (GitHub公開用)
# ==========================================
# GitHubにはAPIキーを上げず、Streamlit Cloudの「Secrets」から読み込みます。
try:
    API_KEY = st.secrets["GEMINI_API_KEY"]
except FileNotFoundError:
    # ローカルPCでテストする時だけ、ここにキーを入れる（GitHubに上げるときは空にする）
    API_KEY = ""

# 🟢 エラー回避: モデルを探さず、安定版を直接指定
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("詳細設定", expanded=False):
            gender = st.radio("性別", ["Male", "Female"], horizontal=True) # 英語にしてグラフエラー回避
    ...
    with tab2:
        ...
        if not favs.empty:
            ...
            st.success(f"{target['name']} : {int(target['kcal'])}kcal (P:{int(target['p'])})")
