...
def analyze_food(text_or_image):
    if not API_KEY:
        ...
        return None
    try:
        # gemini-pro (画像非対応) と gemini-pro-vision (画像対応) の使い分けが必要ですが、
        # エラー回避のため、今回は「テキスト入力なら gemini-pro」のみで確実に動かします。
        ...
        if isinstance(text_or_image, str):
            ...
            # 画像の場合
            model = genai.GenerativeModel("gemini-1.5-flash") # 画像はFlashのみ対応のためトライ
            prompt = "Analyze food from image. Output JSON list: [{'food_name':..., 'calories':..., ...}]"
